...
| Modified       | `Shared/Objects/MediaPlayerManager/MediaPlayerItem/MediaPlayerItem+Build.swift`                           | ✅ Reviewed and actioned | Reviewed live-stream subtitle request handling; no remaining code changes. |
| Modified       | `Shared/Objects/MediaPlayerManager/MediaPlayerItem/MediaPlayerItem.swift`                                 | ✅ Reviewed and actioned | Reviewed live-stream subtitle default selection and track-switch rebuild rules; no remaining code changes. |
| Modified       | `Shared/Objects/MediaPlayerManager/MediaPlayerManager.swift`                                              | ✅ Reviewed and actioned | Reviewed requested playback item state, supplement setup, intro segment loading/cancellation, and skip-intro behavior; no remaining code changes. |
| Modified       | `Shared/Objects/MediaPlayerManager/MediaPlayerProxy/MediaPlayerProxy+VLC.swift`                           | ✅ Reviewed and actioned | Reviewed VLC subtitle index configuration for live and non-live playback; no remaining code changes. |
...
| Modified       | `Shared/Objects/MediaPlayerManager/Supplements/EpisodeMediaPlayerQueue.swift`                             | ✅ Reviewed and actioned | Removed a media-source force unwrap when selecting queue episodes and kept the tvOS selected season synced when playback advances across seasons. |
| Modified       | `Shared/Objects/MediaPlayerManager/Supplements/MediaChaptersSupplement.swift`                             | ✅ Reviewed and actioned | Restored chapter-specific artwork for tvOS chapter cards while keeping the new poster-row overlay. |
| Modified       | `Shared/Objects/MediaPlayerManager/Supplements/MediaInfoSupplement.swift`                                 | ✅ Reviewed and actioned | Reviewed tvOS info overlay layout, program-title handling, and episode show navigation; no remaining code changes. |
| Modified       | `Shared/Objects/MediaPlayerManager/Supplements/MediaPeopleSupplement.swift`                               | ✅ Reviewed and actioned | Reviewed tvOS people poster content path and label behavior; no remaining code changes. |
| Modified       | `Shared/Objects/MediaPlayerManager/Supplements/PlaybackRateMediaPlayerSupplement.swift`                   | ✅ Reviewed and actioned | Reviewed tvOS playback-rate overlay replacement; no remaining code changes. |
| Added          | `Shared/Services/SeerrClient.swift`                                                                       | ✅ Reviewed and actioned | Reviewed request/search/discover/status client flow and localized Seerr status display text. |
...
| Modified       | `Shared/Strings/Strings.swift`                                                                            | ✅ Reviewed and actioned | Reviewed generated localization accessors for the tvOS settings/Seerr request slice and regenerated with SwiftGen. |
...
| Added          | `Shared/Views/SeerrRequestView.swift`                                                                     | ✅ Reviewed and actioned | Localized Seerr request-sheet copy, fallback profile/season labels, and kept partially available TV items requestable for missing seasons. |
| Modified       | `Shared/Views/SettingsView/CustomizeSettingsView.swift`                                                   | ✅ Reviewed and actioned | Reviewed tvOS-only removal of poster-label customization; no remaining code changes. |
| Modified       | `Shared/Views/SettingsView/DebugSettingsView.swift`                                                       | ✅ Reviewed and actioned | Reviewed removal of the debug Liquid Glass toggle; no remaining code changes. |
| Added          | `Shared/Views/SettingsView/SeerrSettingsView.swift`                                                       | ✅ Reviewed and actioned | Localized Seerr settings labels, status/footer copy, editor prompts, and validation failure messaging. |
| Modified       | `Shared/Views/SettingsView/SettingsView.swift`                                                            | ✅ Reviewed and actioned | Localized the tvOS integrations section and Seerr configuration status copy. |
| Modified       | `Shared/Views/SettingsView/VideoPlayerSettingsView.swift`                                                 | ✅ Reviewed and actioned | Reviewed tvOS-only hiding of jump interval and trailing timestamp settings; no remaining code changes. |
...
| Added          | `Swiftfin tvOS Top Shelf/Resources/Swiftfin-tvOS-Top-Shelf.entitlements`                                  | ✅ Reviewed and actioned | Updated the shared app group to `group.org.jellyfin.swiftfin` using the tvOS app identifier as the production base. |
...
| Added          | `Swiftfin tvOS/Resources/Swiftfin-tvOS.entitlements`                                                      | ✅ Reviewed and actioned | Updated the shared app group to `group.org.jellyfin.swiftfin` using the tvOS app identifier as the production base. |
| Added          | `Swiftfin tvOS/Services/TopShelfDeepLinkStore.swift`                                                      | ✅ Reviewed and actioned | —     |
| Added          | `Swiftfin tvOS/Services/TopShelfResumeCache.swift`                                                        | ✅ Reviewed and actioned | Kept the runtime app group constant in sync with the production `group.org.jellyfin.swiftfin` entitlement. |
...
| Modified       | `Swiftfin.xcodeproj/project.pbxproj`                                                                      | ✅ Reviewed and actioned | Updated the Top Shelf extension bundle identifier to `org.jellyfin.swiftfin.TopShelf`, derived from the tvOS app bundle identifier. |
